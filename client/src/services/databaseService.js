...
const client = createClient({
  ...
  authToken: "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3MTQyOTM0MzYsImlkIjoiM2ZiNjJiZTYtYzU4Mi00YzM3LWFhOTctYzdkYzkzYzk3NmRjIn0.1Zn545j3bRleR3TVa2ntRE1GZnHUSHRDCL_h3j8ULZBH_Tj8SWgeT5I4aVElLPVjOYWuCTjtkOQd8f2oE4qYAA",
});
...
export async function insertIntoDatabase(data) {
  ...
  for (const record of data) {
    const { 'DateTime Stamp': date_time, 'Bar OPEN Bid Quote': open_price, 'Bar HIGH Bid Quote': high_price, 'Bar LOW Bid Quote': low_price, 'Bar CLOSE Bid Quote': close_price, 'Volume': volume } = record;
    await executeQuery(insertQuery, [1, 1, date_time, open_price, high_price, low_price, close_price, volume]);
  }
}
...
export async function readCsvFile(filePath) {
  ...
  const records = lines.map(line => {
    ...
    const unixTime = Date.parse(dateTime.slice(0,4) + '-' + dateTime.slice(4,6) + '-' + dateTime.slice(6,8) + 'T' + dateTime.slice(9,11) + ':' + dateTime.slice(11,13) + ':' + dateTime.slice(13,15)) / 1000;
...
export async function addStrategy(strategyName, userId) {
  try {
    ...
    const linkSql = "INSERT INTO userstrategies (user_id, strategy_id) VALUES (?, ?)";
...
export async function addStopLossName(stopLossName, strategyId) {
  try {
    ...
    const linkSql = "INSERT INTO strategystoplosses (strategy_id, stoploss_id) VALUES (?, ?)";
...
export async function addTakeProfitName(takeProfitName, strategyId) {
  try {
    ...
    const linkSql = "INSERT INTO strategytakeprofit (strategy_id, takeprofit_id) VALUES (?, ?)";
...
export async function addIndicatorName(indicatorName, strategyId) {
  try {
    ...
    const linkSql = "INSERT INTO strategyindicator (strategy_id, indicator_id) VALUES (?, ?)";
...
export async function addPricePointName(pricePointName, strategyId) {
  try {
    ...
    const linkSql = "INSERT INTO strategypricepoint (strategy_id, pricepoint_id) VALUES (?, ?)";
...
export async function addAdvancedTrade(strategyId, symbolId, dateStart, entryPrice, stopLossPrice) {
  try {
    // Determine the trade type
    const tradeType = entryPrice >= stopLossPrice ? true : false;

    // Insert the advanced trade
    const tradeSql = "INSERT INTO advancedtrade (strategy_id, pair_id, start_date, entry_price, is_buy) VALUES (?, ?, ?, ?, ?)";
    const trade = await executeQuery(tradeSql, [strategyId, symbolId, dateStart, entryPrice, tradeType]);
    ...
    const defaultStopLossName = stopLossNames.find(name => name.name === 'default stoploss');
    ...
    const stopLossSql = "INSERT INTO StopLoss (price, trade_id, stoploss_name_id) VALUES (?, ?, ?)";
    await executeQuery(stopLossSql, [stopLossPrice, advancedTradeId, stopLossNameId]);
